 ...
 *      978-0-321-35668-0)</a>
 *
 * @authors Gloria Omoba Student 040-919-972
 *
...
public enum HibernateHelper {
    ...
    private HibernateHelper() {
        ...
        try {
            ...
            // TODO
            Configuration config = new Configuration().addAnnotatedClass(Ticket.class)
                    .configure("hibernate.cfg.xml"); // find on cfg file on classpath
            ServiceRegistry serviceR = new StandardServiceRegistryBuilder().applySettings(config.getProperties())
                    .build();
            factory = config.buildSessionFactory(serviceR); // create factory once per application
